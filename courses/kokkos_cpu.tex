...
\AtBeginSection{
    \begin{frame}{Outline}
        \begin{columns}
            ...
            \end{column}
            \begin{column}{0.5\linewidth}
                \tableofcontents[currentsection, sections=10-16]
            \end{column}
        \end{columns}
    \end{frame}
}
...
\begin{frame}{Acknowledgments}
    David Chamont, for his SYCL course which was a great source of inspiration

    \vspace{1em}

    Francesco Rizzi, for his slides for the Kokkos algorithms API
\end{frame}
...
\begin{frame}{What do we want?}
    \begin{itemize}
        \item There are many programming models available
        \item How to choose one?
    \end{itemize}
\end{frame}
...
\begin{frame}{The \textcolor{lightmain}{C}ExA project at CEA}
    \begin{columns}
        \begin{column}{0.8\linewidth}
            \begin{itemize}
                \item \structure{C}omputing at \structure{Ex}ascale with \structure{A}ccelerators at CEA
                \item Started in 2023
                \item Alice Recoque exascale supercomputer in 2027, at CEA
                ...
                \begin{itemize}
                    ...
                    \item Support application demonstrators (CEA codes)
        ...
        \begin{column}{0.2\linewidth}
            ...
            \includegraphics[width=0.8\linewidth]{CExA_logo}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{Kokkos useful resources}
    ...
    \begin{block}{New version incoming!}
        Kokkos v5.2.0 to be released this July
    \end{block}
\end{frame}
...
\begin{frame}{Docker images (CPU images)}
    \begin{itemize}
        ...
        {\tiny \url{ghcr.io/maison-de-la-simulation/gray-scott-kokkos/kokkos_cpu_interactive:latest}}
        \item Jupyter (\texttt{x86\_64}, \texttt{aarch64})

        {\tiny \url{ghcr.io/maison-de-la-simulation/gray-scott-kokkos/kokkos_cpu_interactive_jupyter:latest}}
        \item VSCode (\texttt{x86\_64}, \texttt{aarch64})

        {\tiny \url{ghcr.io/maison-de-la-simulation/gray-scott-kokkos/kokkos_cpu_interactive_vscode:latest}}
        \item CodeServer (\texttt{x86\_64}, \texttt{aarch64})

        {\tiny \url{ghcr.io/maison-de-la-simulation/gray-scott-kokkos/kokkos_cpu_learning_platform_code_server:latest}}
    \end{itemize}
\end{frame}
...
\begin{frame}{Docker images (GPU images)}
    \begin{itemize}
        ...
        {\tiny \url{ghcr.io/maison-de-la-simulation/gray-scott-kokkos/kokkos_gpu_interactive:latest}}
        \item Jupyter (\texttt{x86\_64}, \texttt{aarch64})

        {\tiny \url{ghcr.io/maison-de-la-simulation/gray-scott-kokkos/kokkos_gpu_interactive_jupyter:latest}}
        \item VSCode (\texttt{x86\_64}, \texttt{aarch64})

        {\tiny \url{ghcr.io/maison-de-la-simulation/gray-scott-kokkos/kokkos_gpu_interactive_vscode:latest}}
        \item CodeServer (\texttt{x86\_64}, \texttt{aarch64})

        {\tiny \url{ghcr.io/maison-de-la-simulation/gray-scott-kokkos/kokkos_gpu_learning_platform_code_server:latest}}
    \end{itemize}
\end{frame}

\begin{frame}{What the images provide}
    \begin{columns}[t]
        \begin{column}{0.5\linewidth}
            \begin{center}
                \textbf{CPU and GPU images}
            \end{center}
            \begin{itemize}
                \item Kokkos 5.1.1 sources
                \item Kokkos-tools dev (3d33d0) sources
                \item GCC 14.3.0
                \item HDF5 1.14.6
                \item CLI11 2.6.2
                \item CMake 3.31.8
                \item Exercises folder
            \end{itemize}
        \end{column}
        \pause
        \begin{column}{0.5\linewidth}
    ...
    \begin{columns}
        \begin{column}{0.6\linewidth}
            \inputminted[
                ...
                firstline=8,
                lastline=24,
            ]{sh}{kokkos_cpu_commands.md}
        \end{column}
        \begin{column}{0.4\linewidth}
            \begin{itemize}
                \item Pull the desired image
                \begin{itemize}
                    \item No editor
                    \item Jupyter
                    \item VSCode
...
\section{Gray--Scott sequential implementation}
...
\begin{frame}{The notion of backend in Kokkos}
    \begin{itemize}
        ...
        \item Compilation managed by CMake (no Makefiles!)
    \end{itemize}
    \begin{center}
...
\begin{exerciseframe}{From the "gray\_scott" folder (aka the worktree)}
    \begin{columns}
        \begin{column}{0.5\linewidth}
            \inputminted[
                firstline=78,
                lastline=86,
            ]{C++}{kokkos_cpu_commands.md}
        \end{column}
        \begin{column}{0.5\linewidth}
            \begin{itemize}
                \item Same code as in the \texttt{sequential} folder
...
\begin{exerciseframe}{\small From your worktree or the "cpu\_base\_0\_views" folder}
    \begin{columns}
        \begin{column}{0.5\linewidth}
            \inputminted[
                firstline=94,
...
\begin{exerciseframe}{\small From your worktree or the "cpu\_base\_1\_parallel\_for" folder}
    \begin{columns}
        \begin{column}{0.5\linewidth}
            \inputminted[
                firstline=113,
...
\begin{exerciseframe}{\small From your worktree or the "cpu\_base" folder}
    \begin{columns}
        \begin{column}{0.5\linewidth}
            \inputminted[
                breakafter={=},
...
\begin{frame}{Conclusion}
    \begin{itemize}
        ...
        \item \texttt{parallel\_for}, \texttt{parallel\_reduce}, and \texttt{parallel\_scan} for parallel constructs
