    ...
    \begin{columns}
        ...
        \begin{column}{0.5\linewidth}
            \begin{itemize}
                ...
                \item Rename the CMake list sample file and the C++ file
...
\begin{frame}[fragile]{Anatomy of a Kokkos parallel scan loop}
    \begin{columns}
        ...
        \begin{column}{0.5\linewidth}
            \begin{itemize}
                \item \mintinline{c++}{parallel_scan} to perform reductions
                \item Works only with 1D execution policies
                \item For other types of scan operations, need to provide \texttt{init} and \texttt{join} in a functor (and the function call operator)
                \item Beware, the functor might be called multiple times (\texttt{is\_final})!
            \end{itemize}
        \end{column}
    \end{columns}
\end{frame}
