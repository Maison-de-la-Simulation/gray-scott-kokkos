...
\begin{exerciseframe}{(Re-)Initialization}
    \begin{columns}
        ...
        \begin{column}{0.4\linewidth}
            \begin{itemize}
                \item Clone the Gray-Scott Kokkos repository (if not already)
...
\begin{center}
    \begin{table}{Two Fundamental Types}{ll}
        \textbf{Strong Scaling} & \textbf{Weak Scaling} \\\hline
        Fixed problem size & Problem size grows with processor count \\
        Increase processor count & Work per processor remains constant \\
        Goal: reduce time-to-solution & Goal: maintain runtime while solving larger problems \\
    \end{table}
\end{center}
...
\begin{block}{Definition}
Weak scaling measures how runtime changes when both the number of processors
...
\begin{frame}[fragile]{Example of weak scaling on the Ruche supercomputer}
    \begin{center}
        \includegraphics[width=0.8\textwidth]{weak_scaling_ruche.pdf}
    \end{center}
\end{frame}

\begin{frame}[fragile]{Example of strong scaling on the Ruche supercomputer}
    \begin{center}
...
\begin{columns}
    ...
    \begin{column}{0.5\linewidth}
        \begin{itemize}
            ...
            \item Use Kokkos::Experimental::simd instead of scalars to compute the Gray Scott stencil
            \item The checksum must remain the same!
        \end{itemize}
    \end{column}
\end{columns}
