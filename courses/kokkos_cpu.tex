...
\colorlet{lightmain}{blue} % change me!
...
\author{\scriptsize Mathieu Lobet, Paul Zehner, Paul Gannay, Hariprasad Kannan, Thomas Padioleau, Trévis Morvany}
\institute{CEA}
\date{\today}
\titlegraphic{%
        ...
        \begin{column}{0.6\linewidth}
            \begin{center}
                ...
                \begin{tblr}[theme=kokkostable]{ll}
                    CPU model & Option \\
                    (native) & \texttt{-DKokkos\_ARCH\_NATIVE=ON} \\
...
\begin{frame}{Exercise time}
    \missingfigure{Commands to use}
\end{frame}

\section{Docker images}

\begin{frame}{Exercise time}
    \missingfigure{Description of the images}
\end{frame}
...
\begin{frame}{Exercise time}
    \missingfigure{Build Kokkos and the Hello World code}
\end{frame}

\section{Sequential implementation of the Gray--Scott equation}

\begin{frame}{Presentation time}
    \missingfigure{show the code}
\end{frame}
...
\begin{frame}{Exercise time}
    \missingfigure{Use Views in the CPU folder}
\end{frame}
    ...
    \begin{columns}
        ...
        \begin{column}{0.4\linewidth}
            \begin{itemize}
                ...
                \item Max rank is 6
                \item \texttt{start\_index\_*} and \texttt{end\_index\_*} are the beginning (inclusive) and the end (exclusive) of each loop
            \end{itemize}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{Exercise time}
    \missingfigure{Use parallel loops in the CPU directory}
\end{frame}
...
\begin{frame}{Exercise time}
    \missingfigure{Use parallel reduction in the CPU directory}
\end{frame}

\section{Moving to a CPU backend}
...
\begin{frame}{Exercise time}
    \missingfigure{Build Kokkos and the example in CPU}
\end{frame}

\begin{frame}{Exercise time}
    \missingfigure{Run with and without IO}
\end{frame}

\begin{frame}{Exercise time}
    \missingfigure{Perform strong scaling}
\end{frame}

\begin{frame}{Exercise time}
    \missingfigure{Perform weak scaling}
\end{frame}
    ...
    \begin{itemize}
        ...
        \begin{minted}{C++}
            Kokkos::Experimental::simd<double>
                x(KOKKOS_LAMBDA(int idx) { return idx * idx; });
        \end{minted}
...
\begin{frame}{Exercise time}
    \missingfigure{Vectorize the CPU code}
\end{frame}
...
\begin{frame}[fragile]{Conditionals (bonus)}
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item \structure{Solution:} the \texttt{condition} function, which selects values from two vectors according to a \structure{mask}
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Example: vectorizing with conditionals}
    ...
    \vspace{-10pt}
    \begin{itemize}
        \item \texttt{Kokkos::Experimental::condition} takes a \texttt{simd\_mask}, and two vectors, and selects elements from both vectors depending on the values in the mask
