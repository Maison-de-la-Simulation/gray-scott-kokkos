    ...
    \begin{columns}
        \begin{column}{0.5\linewidth}
            \begin{minted}[breakafter={/}]{sh}
                git clone https://github.com/Maison-de-la-Simulation/gray-scott-kokkos.git

                cd gray-scott-kokkos

                srun ... --pty /bin/bash

                apptainer run ...
            \end{minted}
            \missingfigure{fill out the blanks}
        \end{column}
        \begin{column}{0.5\linewidth}
            \begin{itemize}
                \item Clone the Gray-Scott Kokkos repository
                \item Reserve a job for the day/half a day
                \item Run Apptainer on the image
            \end{itemize}
        \end{column}
    \end{columns}
\end{exerciseframe}
