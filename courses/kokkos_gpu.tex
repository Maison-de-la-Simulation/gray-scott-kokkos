...
\begin{frame}[fragile]{Tiling with Kokkos (bonus)}
    \begin{columns}
        \begin{column}{0.6\linewidth}
            \begin{minted}{C++}
                Kokkos::MDRangePolicy</* rank */>(
                    /* first indices */,
                    /* last indices */,
                    {tile_size_0, /* ... */, tile_size_n}
                );
            \end{minted}
        \end{column}
        \begin{column}{0.4\linewidth}
            \begin{itemize}
                \item Tile size is passed as the third argument of \texttt{MDRangePolicy}
                \item Kokkos has default values depending on the backend
            \end{itemize}
        \end{column}
    \end{columns}
\end{frame}
    ...
    \begin{columns}
        ...
        \begin{column}{0.5\linewidth}
            \begin{itemize}
                ...
                \item \texttt{deep\_copy(destination, source)} dose an automatic fence
            \end{itemize}
        \end{column}
    \end{columns}
\end{frame}
    ...
    \begin{columns}
        ...
        \begin{column}{0.5\linewidth}
            \begin{itemize}
                ...
                \item You may remove unnecessary fences
                \item Observe the performance with Nsight Systems
            \end{itemize}
        \end{column}
    \end{columns}
...
\begin{frame}[fragile]{What about SIMD on GPUs?}
    ...
    \begin{center}
        ...
        \begin{table}{SIMD types}{lll}
            ...
            \texttt{basic\_simd<T, abi<N>>} & \texttt{basic\_simd<T, abi<N>>} & \texttt{basic\_simd<T, scalar>}
        \end{table}
    \end{center}
\end{frame}
    ...
    \begin{columns}
        \begin{column}{0.6\linewidth}
            \begin{table}{Kokkos asynchronism on GPU}{ll}
                ...
                \texttt{parallel\_reduce(/* ... */)} & {\footnotesize Sync.\ (see doc)} \\
    ...
    \begin{columns}
        \begin{column}{0.5\linewidth}
            \begin{minted}{C++}
                ...
                space.fence("wait for stream");
            \end{minted}
        \end{column}
        \begin{column}{0.5\linewidth}
            \begin{itemize}
                \item An instance of an execution space is an object
                \item You can only fence operations queued in it (unlike \mintinline{C++}|Kokkos::fence| that fences all execution spaces)
            \end{itemize}
        \end{column}
    \end{columns}
\end{frame}
    ...
    \begin{columns}
        ...
        \begin{column}{0.4\linewidth}
            \begin{itemize}
                ...
                \item Create one stream for duplication, with \texttt{partition\_space}, to use with \texttt{deep\_copy}
                \item Differentiate the \texttt{deep\_copy} for D-to-D and for D-to-H
                \item Adjust the fences
                \pause
                \item Beware, there is a subtlety on CPU
                \item Make it run correctly on CPU
