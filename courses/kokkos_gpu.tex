    ...
    \begin{columns}
        \begin{column}{0.65\linewidth}
            \begin{minted}[breakafter={/},fontsize=\scriptsize]{sh}
                ...
                # pick one
                apptainer run --nv $REPO/kokkos_gpu_interactive
                apptainer run --nv $REPO/kokkos_gpu_interactive_jupyter
                apptainer run --nv $REPO/kokkos_gpu_interactive_vscode
                apptainer run --nv $REPO/kokkos_gpu_learning_platform_code_server

                # if needed (you shouldn't have to)
                cp -rn /home/jovyan/Examples $HOME/gray_scott_kokkos
...
\begin{frame}[fragile]{Race condition (bonus)}
...
\begin{frame}{Kokkos ekosystem (bonus)}
    \begin{center}
    ...
    \begin{columns}
        \begin{column}{0.8\linewidth}
            \begin{minted}[fontsize=\tiny]{C++}
                // Handle wrapping Kokkos exec space + comm object (e.g. MPI_Comm, ncclComm_t, etc.)
                ...
                // Initialize local part of vector x
                Kokkos::parallel_for(
                    "init local x",
                ...
                // Initiate in-place all-gather communication of x
                ...
                // Wait for all-gather of x to finish
