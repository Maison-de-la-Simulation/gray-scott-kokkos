...
\begin{frame}{Exercise time}
    \missingfigure{compile Kokkos with a GPU backend}
\end{frame}

\begin{frame}{Exercise time}
    \missingfigure{compile the CPU code with a GPU backend}
\end{frame}
...
\begin{frame}{View with SharedSpace memory space}
    ...
    \begin{itemize}
        \item If Kokkos is compiled with a \structure{GPU backend only}, the View data is allocated in the \structure{Device memory}
        \item Shared view data \structure{can} be accessed from the Host, as it is copied in chunks automatically by the OS and the driver
        \item Performance cost
    \end{itemize}
\end{frame}

\begin{frame}{Exercise time}
    \missingfigure{use the shared space in the CPU example}
\end{frame}
...
\begin{frame}{Exercise time}
    \missingfigure{add fence to the CPU code with shared space}
\end{frame}
...
\begin{frame}{Exercise time}
    \missingfigure{add regions in the code and profile it; see the performance of the shared space}
\end{frame}
...
\begin{frame}{Exercise time}
    \missingfigure{add mirrors and deep copy to the cpu code}
\end{frame}
...
\begin{frame}{Exercise time}
    \missingfigure{run the SIMD code on GPU with a SharedSpace View and a fence}
\end{frame}
    ...
    \begin{columns}
        \begin{column}{0.4\linewidth}
            ...
            \begin{enumerate}
                ...
                \item Synchronization \structure{(GPU-CPU)}
                \item Writing \structure{(CPU)}, slow because of the filesystem
            \end{enumerate}
        \end{column}
        \pause
        \begin{column}{0.6\linewidth}
            ...
            \begin{tikzpicture}[node distance=1em]
                ...
                \node[below=of compute] (synchronize) {synchronize};
                \node[below=of synchronize] (write) {write};

                \draw[->] (compute) -- (synchronize);
                \draw[->] (synchronize) -- (write);
...
\begin{frame}{Exhibit asynchronism in the workflow}
    \begin{columns}
        ...
        \begin{column}{0.5\linewidth}
            ...
            \begin{tikzpicture}[node distance=1em]
                ...
                \draw[->] (synchronize) -- (compute-after);

                \node[above=of write-before] (ellipsis-before) {...};
                \node[below=of compute-after] (ellipsis-after) {...};

                \draw[->] (ellipsis-before) -- (compute);
                \draw[->] (write-before) -- (synchronize);
                \draw[->] (ellipsis-before) -- (write-before);
                \draw[->] (compute-after) -- (ellipsis-after);
    ...
    \end{columns}
    \begin{block}{Idea}
        Synchronization can be synchronous
    \end{block}
\end{frame}
...
\begin{frame}{What is the current workflow?}
        ...
        \begin{column}{0.4\linewidth}
            ...
            \begin{enumerate}
                \item Synchronization \structure{(GPU-CPU)}, slow because of the PCIe bus
                \item 10 computations \structure{(GPU)}
                \item Writing \structure{(CPU, hidden)}
            \end{enumerate}
        \end{column}
        \pause
        \begin{column}{0.6\linewidth}
...
\begin{frame}{Exhibit more asynchronism in the workflow}
    \begin{columns}
        ...
        \begin{column}{0.6\linewidth}
            ...
            \begin{tikzpicture}[node distance=1em]
                \node (compute) {compute $\times$ 10};
                \node[below=3em of compute] (duplicate) {duplicate};
                \node[below=of duplicate] (synchronize) {synchronize};
                \node[below=of synchronize] (write) {write};

                \draw[->] (compute) -- (duplicate);
                \draw[->] (duplicate) -- (synchronize);
                \draw[->] (synchronize) -- (write);

                \node[left=of compute] (synchronize-before) {synchronize};
                \node[below=of synchronize-before] (write-before) {write};
                \node[right=of synchronize] (compute-after) {compute $\times$ 10};

                \draw[->] (duplicate) -- (compute-after);

                \node[above=of synchronize-before] (ellipsis-before) {...};
                \node[below=3em of compute-after] (ellipsis-after) {...};

                \draw[->] (ellipsis-before) -- (compute);
                \draw[->] (write-before) -- (duplicate);
                \draw[->] (ellipsis-before) -- (synchronize-before);
                \draw[->] (synchronize-before) -- (write-before);
                \draw[->] (compute-after) -- (ellipsis-after);
                \draw[->] (write) -- (ellipsis-after);
            \end{tikzpicture}
        \end{column}
    \end{columns}
    \begin{block}{Idea}
        D-to-H synchronize can be viewed as D-to-D duplicate, then D-to-H synchronize
    \end{block}
\end{frame}
    ...
    \begin{columns}
        ...
        \begin{column}{0.4\linewidth}
            \begin{itemize}
                ...
                \item Create one stream for computation, and one for duplication
                \begin{itemize}
                    \item One stream for \texttt{parallel\_for}
                    \item One stream for \texttt{deep\_synchronize}
                \end{itemize}
                \item Differentiate the \texttt{deep\_synchronize} for the duplication and for the synchronization
                \item Adjust the fences
            \end{itemize}
        \end{column}
    \end{columns}
...
\begin{frame}{Exercise time}
    \missingfigure{hide data transfer too}
\end{frame}
...
\begin{frame}[plain]
    ...
    \end{center}
    \vspace{2em}
    \begin{description}[Gray-Scott Kokkos repository]
