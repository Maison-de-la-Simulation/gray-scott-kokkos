...
\begin{frame}{View with \texttt{Kokkos::SharedSpace} memory space}
    \begin{center}
        \includegraphics[width=0.65\textwidth]{shared_view_memory.png}
    \end{center}
...
\begin{frame}{Kokkos tools}
    \begin{itemize}
        ...
        Mind the \texttt{S}!
        \item Or with the \texttt{--kokkos-tools-libs} argument of the program
...
\begin{frame}{How to access data living on the Host from the Host?}
    ...
    \structure{Note:} For portability reason, we always use a mirror view whether we compile with a CPU or GPU backend
\end{frame}
